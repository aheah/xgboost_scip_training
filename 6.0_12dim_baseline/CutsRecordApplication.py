...
def cuts_record(mps_file):
    ...
    basePath = r'D:\\Develop\\graduate_student\\experiment_data\\milp_final_model_data\\code\\6.0_12dim_baseline\\'
    ...
    useful_cols = [
        'node_id', 'round_id', 'cut_name', 'cut_type', 'cut_efficacy',
        'var_count', 'non_zero_coefs', 'row_norm', 'is_local', 'parallelism',
        'max_coef', 'min_coef', 'avg_coef', 'isp', 'obp', 'dcd',
        'timestamp', 'dual_bound', 'gap'
    ]

    # --- 1. 保存全部的约束数据 ---
    if eventHandler.all_row_data:
        cut_file = os.path.join(data_dir, f"separator_all_row_data_{timestamp}.csv")
        df_all = pd.DataFrame(eventHandler.all_row_data)
        cols_to_save = [c for c in useful_cols if c in df_all.columns]
        df_all[cols_to_save].to_csv(cut_file, index=False)
        print(f"✓ 保存全部的约束数据 (已精简): {len(eventHandler.all_row_data)} 条")

    # --- 2. 保存割分离器的切割数据 ---
    if eventHandler.separator_data:
        cut_file = os.path.join(data_dir, f"separator_data_{timestamp}.csv")
        df_sep = pd.DataFrame(eventHandler.separator_data)
        cols_to_save = [c for c in useful_cols if c in df_sep.columns]
        df_sep[cols_to_save].to_csv(cut_file, index=False)
        print(f"✓ 保存割分离器的切割数据 (已精简): {len(eventHandler.separator_data)} 条")

    # --- 3. 保存割选择器的切割数据 ---
    if cutSelector.selected_data:
        select_data_file = os.path.join(data_dir, f"separator_selector_data_{timestamp}.csv")
        df_sel = pd.DataFrame(cutSelector.selected_data)
        cols_to_save = [c for c in useful_cols if c in df_sel.columns]
        df_sel[cols_to_save].to_csv(select_data_file, index=False)
        print(f"✓ 保存割选择器的切割数据 (已精简): {len(cutSelector.selected_data)} 条")

    # --- 4. 保存加入LP的切割数据 ---
    if eventHandler.separator_lp_data:
        cut_file = os.path.join(data_dir, f"separator_lp_data_{timestamp}.csv")
        df_lp = pd.DataFrame(eventHandler.separator_lp_data)
        cols_to_save = [c for c in useful_cols if c in df_lp.columns]
        df_lp[cols_to_save].to_csv(cut_file, index=False)
        print(f"✓ 保存加入LP的切割数据 (已精简): {len(eventHandler.separator_lp_data)} 条")
    ...
    if eventHandler.gap_data:
        ...
        pd.DataFrame(eventHandler.gap_data).to_csv(gap_file, index=False)
        print(f"✓ 保存间隙数据: {len(eventHandler.gap_data)} 条")

    if eventHandler.node_data:
        node_file = os.path.join(data_dir, f"node_{timestamp}.csv")
        pd.DataFrame(eventHandler.node_data).to_csv(node_file, index=False)
...
if __name__ == "__main__":
    ...
    for problem in difficult_problems:
        mps_full_path = os.path.join(
            r'D:\\Develop\\graduate_student\\experiment_data\\milp_final_model_data\\code\\6.0_12dim_baseline\\',
            problem)

        if os.path.exists(mps_full_path):
            print("=" * 100)
            ...
            try:
                ...
                print(f"❌ 严重警告：模型 {problem} 运行崩溃，已强制跳过！错误原因: {e}")
            ...
            # 🚨 加上这一句，找不到文件立刻报警！
            print(f"⚠️ 警报：找不到文件 {mps_full_path}，已跳过！请把 MPS 文件复制到该目录下。")
