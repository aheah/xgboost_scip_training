...
def cuts_record(mps_file):
    ...
    basePath = r'D:\\Develop\\graduate_student\\experiment_data\\milp_final_model_data\\code\\3.0\\'
    ...
    if eventHandler.all_row_data:
        ...
        pd.DataFrame(eventHandler.all_row_data).to_csv(cut_file, index=False)
    ...
    if eventHandler.separator_data:
        ...
        pd.DataFrame(eventHandler.separator_data).to_csv(cut_file, index=False)
    ...
    if cutSelector.selected_data:
        ...
        pd.DataFrame(cutSelector.selected_data).to_csv(select_data_file, index=False)
    ...
    if eventHandler.separator_lp_data:
        ...
        pd.DataFrame(eventHandler.separator_lp_data).to_csv(cut_file, index=False)
...
if __name__ == "__main__":
    ...
    for problem in difficult_problems:
        ...
        if os.path.exists(os.path.join(r'D:\\Develop\\graduate_student\\experiment_data\\milp_final_model_data\\code\\3.0\\', problem)):
            print("=" * 100)
