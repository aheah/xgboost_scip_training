...
# 🧠 核心提取器 (保持不变)
# ==========================================
def extract_12_ultimate_features(model, cut):
    # ==========================================
    # 1. 原有的 9 个基础特征 (保持原汁原味)
    # ==========================================
    try:
    ...
    var_count, non_zero_coefs, row_norm = 0, 0, 0.0
    max_coef, min_coef, avg_coef = 0.0, 0.0, 0.0
    is_local = 0

    # ==========================================
    # 2. 新增的 3 个顶级特征 (dcd, obp, isp)
    # ==========================================
    dcd = 0.0  # 定向截断距离 (Directed Cutoff Distance)
    obp = 0.0  # 目标平行性 (Objective Parallelism)
    isp = 0.0  # 整数支持度 (Integer Support)
    ...
    try:
        ...
        if cols and coefs:
            ...
            if valid_coefs:
                ...
                avg_coef = sum(valid_coefs) / len(valid_coefs)

            # 🌟 新增计算 1：isp (整数支持度)
            # 遍历这把刀里的所有变量，数一数有几个是整数变量 (BINARY 或 INTEGER)
            int_var_count = 0
            for col in cols:
                var = col.getVar()
                # 获取变量类型：'B' 是 0-1 变量，'I' 是整数变量
                if var.vtype() in ['BINARY', 'INTEGER']:
                    int_var_count += 1
            # 算比例：整数变量占比越高，割平面越稳定
            isp = int_var_count / var_count if var_count > 0 else 0.0

        row_norm = cut.getNorm()
        is_local = int(cut.isLocal())

        # 🌟 新增计算 2 & 3：obp 和 dcd
        # 注意：PySCIPOpt 的版本不同，API 名字可能微调，必须用 try-except 护体
        try:
            obp = cut.getObjParallelism()
        except:
            obp = 0.0

        try:
            # SCIP 底层评估割平面距离目标函数的截断距离
            dcd = model.getCutLPSolCutoffDistance(cut)
        except:
            dcd = 0.0

    except Exception as e:
        # 如果遇到畸形割平面，直接 pass，全部赋默认值 0
        pass

    # 严格按照顺序返回这 12 个特征！
    return [
        efficacy, parallelism, var_count, non_zero_coefs, row_norm,
        max_coef, min_coef, avg_coef, is_local,
        dcd, obp, isp  # 👈 这是你最新加入的“三大金刚”
    ]
...
class XGBoostCutSelector(Cutsel):
    ...
    def cutselselect(self, cuts, forcedcuts, root, maxnselectedcuts):
        try:
            ...
            rejected_cuts = []
            feature_names = [
                'cut_efficacy', 'parallelism', 'var_count', 'non_zero_coefs', 'row_norm',
                'max_coef', 'min_coef', 'avg_coef', 'is_local',
                'dcd', 'obp', 'isp'  # 👈 加上这三个列名
            ]
            ...
            for i, cut in enumerate(cuts):
                if cut.getOrigintype() == 3:
                    ...
                    batch_features.append(extract_12_ultimate_features(self.model, cut))
